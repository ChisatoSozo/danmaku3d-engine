...
import { times } from "../../containers/GlobalUniforms";
...
import "./Behaviours";
import { MAX_VALUES_PER_FRAME } from "./Behaviours";
import { DifferentialTexture } from "./DifferentialTexture";
import { makeTextureFromVectors } from "./utils";
...
export class CustomParticleSystemEngine {
    ...
    constructor(
        public settings: CustomParticleSystemEngineSettings = {
            count: 5000,
            minLifespan: 0.3,
            maxLifespan: 1.5,
            minSize: 0.08,
        ...
        const positions = settings.initialPositions
            ? settings.initialPositions
            : times(settings.count, () => new Vector3(-100000, -100000, -100000));
        const velocities = times(settings.count, () =>
            new Vector3(
                randomRange(settings.direction1.x, settings.direction2.x),
                randomRange(settings.direction1.y, settings.direction2.y),
                randomRange(settings.direction1.z, settings.direction2.z)
            ).scale(randomRange(settings.minVelocity, settings.maxVelocity))
        );
        ...
        const setInitialUniforms = (texture: CustomFloatProceduralTexture) => {
            ...
            texture.setFloats(
                "setValuesFloats",
                times(MAX_VALUES_PER_FRAME * 3, () => 0)
            );
            ...
            texture.setVector3(
                "gravity",
                settings.gravity || new Vector3(0.0000000000000000001, 0.0000000000000000001, 0.0000000000000000001)
            );
            texture.setVector3(
                "offset",
                new Vector3(0.0000000000000000001, 0.0000000000000000001, 0.0000000000000000001)
            );
        ...
        this.positionTexture = new DifferentialTexture(
            initialPositionsTexture,
            "WFVRlinearBehaviourPosition",
            scene,
            setInitialUniforms
        );
        this.velocityTexture = new DifferentialTexture(
            initialVelocitiesTexture,
            targetsTexture ? "WFVRtargetBehaviourVelocity" : "WFVRlinearBehaviourVelocity",
            scene,
            setInitialUniforms
        );
    ...
    getEmitValues(count: number) {
        ...
        return vectors.map((v) => [v.x, v.y, v.z]).flat();
    }

    getEmitVelocityValues(count: number) {
        const vectors = times(count, () =>
            new Vector3(
                randomRange(this.settings.direction1.x, this.settings.direction2.x),
                randomRange(this.settings.direction1.y, this.settings.direction2.y),
                randomRange(this.settings.direction1.z, this.settings.direction2.z)
            ).scale(randomRange(this.settings.minVelocity, this.settings.maxVelocity))
        );
        //return vectors as an array of floats
        return vectors.map((v) => [v.x, v.y, v.z]).flat();
    }
