...
export class DifferentialTexture {
    ...
    constructor(
        initialValues: RawTexture | Vector3[],
        updateShader: string,
        scene: Scene,
        setInitialUniforms?: (texture: CustomFloatProceduralTexture) => void
    ) {
        const WIDTH =
            initialValues instanceof Texture
                ? initialValues.getSize().width
                : nextPowerOfTwo(Math.sqrt(initialValues.length));

        if (WIDTH > 1024) throw new Error("Too many particles");
        this.initialTexture =
            initialValues instanceof Texture ? initialValues : makeTextureFromVectors(initialValues, scene);
        this.textures = [
            makeProceduralTexture("texture1", updateShader, WIDTH, scene),
            makeProceduralTexture("texture2", updateShader, WIDTH, scene),
        ];
