...
export type Keys = "MENU" | "ENTER" | "DOWN" | "UP" | "LEFT" | "RIGHT" | "SLOW" | "BOMB" | "SHOOT" | "DIALOGUE";

const defaultKeyMap: { [key: number]: Keys } = {
    27: "MENU",
    13: "ENTER",
    40: "DOWN", //Down arrow
    83: "DOWN", //s
...
const makeDefaultDownKeyMap = () => {
    ...
    const downKeyMap: { [key: string]: boolean } = {};
    keys.forEach((key) => {
        downKeyMap[key] = false;
    });
    return downKeyMap as KeyDown;
};
...
export const keyObject: KeyObject = {
    ...
    disabledMap: makeDefaultDownKeyMap(),
};
...
export interface IControlsContext {
    ...
    disableControl: (control: Keys) => void;
    enableControl: (control: Keys) => void;
...
const defaultControlsContext: IControlsContext = {
    ...
    downKeys: makeDefaultDownKeyMap(),
    keyDownHandler: () => {
        return;
    },
    keyUpHandler: () => {
...
export const ControlsContext = React.createContext<IControlsContext>(defaultControlsContext);

export const useControlsContext = (outsideOfRenderer: boolean): IControlsContext => {
    const [downKeys, setDownKeys] = useState<KeyDown>(makeDefaultDownKeyMap());
    ...
    const keyDownHandler = useCallback(
        (event: DanmakuControlEvent) => {
            ...
            const key = keyMap[event.which] as Keys;
    ...
    const disableControl = useCallback((control: Keys) => {
        keyObject.disabledMap[control] = true;
    }, []);

    const enableControl = useCallback((control: Keys) => {
